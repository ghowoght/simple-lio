本文中变量命名与Fast-LIO论文中不尽相同，比如为了便于阅读而忽略了很多上下标，如有误解之处，敬请指教。
...
接下来推导误差状态向量$\delta x$，定义误差状态变量如下：
$$
p=\hat p+\delta p\\
v=\hat v+\delta v\\
R=R\delta R\\
...
g=\hat g+\delta g
$$
姿态部分的误差$\delta R$对应的李代数为$\delta\theta$，即$\delta R=Exp(\delta\theta)$。因此三者的关系可以表述为：
$$
...
\delta g_{k+1}=g_k
$$
上式可以记为$\delta x_{k+1}=F_x\delta x_k+F_ww$，其中$w=\begin{bmatrix}n_v^T&n_\theta^T&n_g^T&n_a^T\end{bmatrix}^T\sim N(0,Q)$
$$
F_x=\begin{bmatrix}
...
$$
由于$\delta x$在更新后都会被重置为0，因此可以不进行计算。

## 观测模型

### 观测值

以平面特征点的观测方程为例，首先将lidar系下的点转换到world系下：
$$
p_w=Rp_l+t
$$
其中忽略了lidar和imu的外参以及运动补偿。

然后从地图中找到5个与$p_w$对应的平面特征点，用这些点拟合平面$Ax+By+Cz+D=0$，也即$\frac{A}{D}x+\frac{B}{D}y+\frac{C}{D}z=-1$。求解得到归一化法向量$u=[A,B,C]^T$和截距$D$。然后计算点到平面的距离$z=Ax+By+Cz+D$。

通过以上步骤，可以得到点$p_w$对应的平面归一化法向量$u$和点到平面的距离$z$。

### 观测方程

如果使用真实的参数$R,t$，计算得到的点到平面的距离应该为0，也就是:
$$
\begin{aligned}
h(x)
...
\end{aligned}
$$
分别求在$\delta{\theta}=0$处和$\delta t=0$处的偏导$\frac{\partial H\delta x}{\partial\delta \theta}$和$\frac{\partial H\delta x}{\partial\delta t}$，得到：
$$
\begin{aligned}
...
由于$\begin{cases}z+H\delta x_k\sim N(0,Q)\\d+J_k\delta x_k\sim N(0,P)\end{cases}$均服从高斯分布，**最大后验估计**等价于最小化**贝叶斯风险(BMSE)**，即：
$$
\mathop{min}_{\delta x_k} ||d+J_k\delta x_k||^2_{P^{-1}}+||z_k+H\delta x_k||^2_{R^{-1}}
$$
将目标函数$\epsilon$表示如下：
$$
\begin{aligned}
...
[https://github.com/hku-mars/FAST_LIO/issues/28](https://github.com/hku-mars/FAST_LIO/issues/28)
